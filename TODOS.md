...
## Set up Google Calendar OAuth (blocking Calendar push)
**What:** Create a Google Cloud project, enable the Calendar API, create an OAuth client (type: Desktop app), then run `node scripts/get-google-refresh-token.mjs` locally with the client ID/secret to get a refresh token. Add `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN` to your env (local + Vercel).
**Why:** `lib/calendar/pushDailyQueue.ts` is written and typechecks, but has never been called against a real Google account — needs this setup before it can be tested end-to-end. The daily cron job in `/api/push/notify` already skips the Calendar step entirely if these env vars are unset, so this is safe to leave undone; Calendar push just won't run until it's configured.
**Heads up:** the same "Testing" publish-status 7-day refresh-token expiry that was originally flagged for the (now-dropped) YouTube integration also applies here, since this is a separate OAuth client on the same kind of unverified consent screen. Accept it for now (same reasoning as the obsolete YouTube TODO above — not worth Google's verification process before you know you're using this daily), but expect to re-run the script roughly weekly until/unless you revisit this.
**Depends on / blocked by:** nothing — can be done anytime, independent of the rest of Phase 1.
